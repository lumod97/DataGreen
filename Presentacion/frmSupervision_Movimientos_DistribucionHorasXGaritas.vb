...
Public Class frmSupervision_Movimientos_DistribucionHorasXGaritas
    Dim dtTablaExcel As New DataTable
    Dim arrayDeParametros As String = String.Empty
    Dim tablaResultado As DataTable

    Private Sub frmSupervision_Movimientos_DistribucionHorasXGaritas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        aplicarTema(Me)
    ...
    Private Async Function ProcesarAsync() As Task(Of DataTable)
        Try
            ...
            r = Await Task.Run(Function() doItBaby("sp_Dg_Supervision_Movimientos_DistribucionHorasXGaritas", p, TipoQuery.DataTable))
            If Len(r.Rows(0).Item(0)) > 0 Then
                MessageBox.Show("Distribucion Correcta!")
        ...
        '        Dim r As New DataTable
        '        arrayDeParametros = obtenerCadenaParametros(p)
        '        r = Await Task.Run(Function() doItBaby("sp_Dg_Almacenes_Movimientos_CargarPedidosCompra", p, TipoQuery.DataTable))
        '        If Len(r.Rows(0).Item(0)) > 0 Then
