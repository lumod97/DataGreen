...
Public Class frmCostos_Reportes_HorasMaquinaXActividad
    Dim arrayDeParametros As String = String.Empty
    Dim tablaResultado As DataTable
    Dim dsDataParaCombo As DataSet

    Private Sub frmCostos_Reportes_HorasMaquinaXActividad_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        tlpPrincipal.Visible = False
        Me.WindowState = FormWindowState.Maximized
        obtenerDataParaCombos()
        cargarCombo(cboMaquina, dsDataParaCombo.Tables(0), "", True)
        cargarCombo(cboActividad, dsDataParaCombo.Tables(1), "", True)
    ...
    Private Async Function consultarAsync() As Task(Of DataTable)
        Try
            ...
            p.Add("@IdMaquina", cboMaquina.SelectedValue)
            p.Add("@IdActividad", cboActividad.SelectedValue)
            p.Add("@Detalle", cbxDetalle.Checked)
            arrayDeParametros = obtenerCadenaParametros(p)
            aux = Await Task.Run(Function() doItBaby("sp_Dg_Costos_Reportes_HorasMaquinaXActividad", p, TipoQuery.DataTable))
            Return aux
    ...
    Private Sub obtenerDataParaCombos()
        ...
        dsDataParaCombo.Tables.Add(doItBaby("sp_Dg_ObtenerMaquinas", Nothing, TipoQuery.DataTable))
