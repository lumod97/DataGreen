...
Public Class frmRrhh_Reportes_ResumenPlanillas
    Dim arrayDeParametros As String = String.Empty
    Dim tablaResultado As DataTable
    Dim dsDataParaCombo As DataSet

    Private Sub frmRrhh_Reportes_ResumenPlanillas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        tlpPrincipal.Visible = False
        Me.WindowState = FormWindowState.Maximized
        obtenerDataParaCombos()
        cargarCombo(cboTipoProceso, dsDataParaCombo.Tables(0), "", False)
    ...
    Private Async Function consultarAsync() As Task(Of DataTable)
        Try
            ...
            p.Add("@CodPlanilla", txtPlanilla.Text)
            p.Add("@Periodo", txtPeriodo.Text)
            p.Add("@Semana", txtSemana.Text)
            p.Add("@TipoProceso", cboTipoProceso.SelectedValue)
            arrayDeParametros = obtenerCadenaParametros(p)
            aux = Await Task.Run(Function() doItBaby("sp_Dg_Rrhh_Reportes_ResumenPlanillas", p, TipoQuery.DataTable))
            Return aux
    ...
    Private Sub obtenerDataParaCombos()
        ...
        dsDataParaCombo.Tables.Add(doItBaby("sp_Dg_ObtenerTiposProcesoPlanillas", Nothing, TipoQuery.DataTable))
