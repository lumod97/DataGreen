...
Public Class frmCostos_Reportes_ReporteServicios
    Dim dsDataParaCombos As New DataSet
    Dim tablaResultado As DataTable
    Dim arrayDeParametros As String = String.Empty

    Private Sub frmCostos_Reportes_ReporteServicios_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        tlpPrincipal.Visible = False
        Me.WindowState = FormWindowState.Maximized
        aplicarTema(Me)
        obtenerDataParaCombos()
        cargarCombo(cboPeriodoInicial, dsDataParaCombos.Tables(0), "", False)
    ...
    Private Sub obtenerDataParaCombos()
        ...
        p.Add("@Desde", "")
        dsDataParaCombos.Tables.Add(doItBaby("sp_Dg_Costos_Reportes_ReporteServicios_ObtenerData", p, TipoQuery.DataTable))
        dsDataParaCombos.Tables.Add(New DataTable)
    End Sub

    Private Sub cboPeriodoInicial_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles cboPeriodoInicial.SelectionChangeCommitted
        Dim p As New Dictionary(Of String, Object)
        p.Add("@Desde", cboPeriodoInicial.SelectedValue)
        dsDataParaCombos.Tables.RemoveAt(1)
        dsDataParaCombos.Tables.Add(doItBaby("sp_Dg_Costos_Reportes_ReporteServicios_ObtenerData", p, TipoQuery.DataTable))
        cargarCombo(cboPeriodoFinal, dsDataParaCombos.Tables(1), "", False)
    End Sub
    ...
    Private Async Function consultarAsync() As Task(Of DataTable)
        Try
            ...
            Dim p As New Dictionary(Of String, Object)
            p.Add("@Periodo_A", cboPeriodoInicial.SelectedValue)
            p.Add("@Periodo_Z", cboPeriodoFinal.SelectedValue)
            arrayDeParametros = obtenerCadenaParametros(p)
            aux = Await Task.Run(Function() doItBaby("sp_Dg_Costos_Reportes_ReporteServicios", p, TipoQuery.DataTable))
            Return aux
