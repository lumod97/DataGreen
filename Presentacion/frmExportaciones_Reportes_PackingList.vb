...
Public Class frmExportaciones_Reportes_PackingList
    Dim arrayDeParametros As String = String.Empty
    Dim tablaResultado As DataTable
    Dim dsDataParaCombo As DataSet

    Private Sub frmExportaciones_Reportes_PackingList_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        tlpPrincipal.Visible = False
        Me.WindowState = FormWindowState.Maximized
        dtpDesde.Value = DateAdd(DateInterval.Day, -7, dtpHasta.Value)
        aplicarTema(Me)
    ...
    Private Async Function consultarAsync() As Task(Of DataTable)
        Try
            ...
            aux = Await Task.Run(Function() doItBaby("sp_Dg_Exportaciones_Reportes_PackingList_ListarPackingLists", p, TipoQuery.DataTable))
            Return aux
    ...
    Private Sub dgvResultado_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvResultado.CellDoubleClick
        If e.RowIndex >= 0 AndAlso e.ColumnIndex >= 0 Then
            abrirDetalle(dgvResultado.Rows(e.RowIndex).Cells("IdPackingList").Value)
        End If
    End Sub

    Private Sub abrirDetalle(idPackingList As String)
        Dim VenActual As New frmExportaciones_Reportes_PackingList_Detalle(idPackingList)
